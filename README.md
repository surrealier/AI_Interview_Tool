...
권장 환경은 Windows, Python 3.12입니다. 기본 TTS 엔진은 Windows 기본 음성이라 별도 런타임 의존성 없이 실행됩니다.
...
Qwen3-TTS는 선택 기능입니다. GPU/CUDA 환경이 준비되어 있고 앱의 `TTS 엔진`에서 `Qwen3-TTS`를 선택할 때만 필요합니다.
...
- Qwen3-TTS가 실패하면 앱은 기본값으로 Windows SAPI 음성 fallback을 사용해서 WAV를 만들고 재생합니다.
- 현재 기본 TTS 엔진은 바로 소리가 나는 `Windows 기본 음성`입니다. Qwen3-TTS를 시험하려면 앱의 `TTS 엔진`에서 `Qwen3-TTS`를 직접 선택하세요.
